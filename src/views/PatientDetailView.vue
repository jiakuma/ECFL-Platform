...
// ---------- 中部功能导航 ----------
type NavKey = 'overview' | 'journey' | 'exams' | 'pathology' | 'treatment' | 'followup' | 'archive'
const navItems: { key: NavKey; label: string }[] = [
  { key: 'overview', label: '患者概览' },
  { key: 'journey', label: '患者旅程' },
  { key: 'exams', label: '检查检验' },
  { key: 'pathology', label: '病理与分子' },
  { key: 'treatment', label: '治疗记录' },
  { key: 'followup', label: '随访预后' },
  { key: 'archive', label: '完整档案' },
]
const activeNav = ref<NavKey>('overview')

// ---------- 患者旅程时间轴（核心） ----------
const timelineKinds = ['全部', '诊断', '检查', '治疗', '随访'] as const
const timelineFilter = ref<(typeof timelineKinds)[number]>('全部')
const filteredTimeline = computed(() =>
  patient.value.timeline.filter((t) => timelineFilter.value === '全部' || t.kind === timelineFilter.value),
)
const kindColor: Record<string, string> = {
  建档: '#2563eb', 诊断: '#8b5cf6', 治疗: '#06b6d4', 随访: '#10b981', 检查: '#f59e0b',
}
// 最近事件 = 时间轴首位（数据倒序）；当前阶段 = 进行中的治疗节点
const recentNodeId = computed(() => filteredTimeline.value[0]?.id ?? '')
...
const currentStageNodeId = computed(() => {
  ...
  if (!ct) return ''
  const hit = patient.value.timeline.find((n) => n.kind === '治疗' && n.title.includes(ct.name))
  return (hit ?? patient.value.timeline.filter((n) => n.kind === '治疗')[0])?.id ?? ''
})

// ---------- 全局临床摘要（顶部共享，原右侧四卡升级） ----------
const treatmentPhaseText = computed(() =>
  currentTreatment.value ? `${currentTreatment.value.line} · ${currentTreatment.value.modality}` : '暂无进行中治疗',
)
...
<template>
  <div class="pd">
    ...
    <!-- ===================== 主体：默认单栏；患者旅程双栏 ===================== -->
    <div class="pd__main" :class="{ 'pd__main--journey': activeNav === 'journey' }">
      <div class="pd__left">
        <!-- ① 患者概览：信息组（非表格） -->
        <div v-if="activeNav === 'overview'">
          <div class="pd__group">
            <h3 class="pd__h">基本信息</h3>
            <dl class="pd__kv">
              <dt>患者姓名</dt><dd>{{ displayName }}</dd>
              <dt>性别 / 年龄</dt><dd>{{ patient.gender }} / {{ patient.age }} 岁</dd>
              <dt>证件号码</dt><dd><span class="pd__masked">{{ masked.idNo }}<el-icon><Lock /></el-icon></span></dd>
              <dt>联系电话</dt><dd><span class="pd__masked">{{ masked.phone }}<el-icon><Lock /></el-icon></span></dd>
              <dt>常住地址</dt><dd><span class="pd__masked">{{ masked.address }}<el-icon><Lock /></el-icon></span></dd>
              <dt>建档时间</dt><dd>{{ patient.joinedAt }}</dd>
              <dt>数据所属机构</dt><dd>{{ patient.org }}</dd>
            </dl>
          </div>
          <div class="pd__group">
            <h3 class="pd__h">主索引映射</h3>
            <dl class="pd__kv">
              <dt>平台主索引</dt>
              <dd><span class="pd__mono">{{ patient.primaryId }}</span></dd>
            </dl>
            <div class="pd__map">
              <div v-for="m in patient.localIds" :key="m.id" class="pd__map-row">
                ...
                <el-tag size="small" effect="plain" type="info">主索引自动映射</el-tag>
              </div>
            </div>
          </div>
          <div class="pd__group">
            <h3 class="pd__h">临床摘要</h3>
            <dl class="pd__kv">
              <dt>主要诊断</dt><dd>{{ patient.diagnosis }}</dd>
              <dt>TNM 分期</dt><dd><span class="pd__stage-pill sm">{{ patient.stage }}</span></dd>
              <dt>当前临床状态</dt>
              <dd>
                <span class="pd__status-pill sm" :style="{ background: statusColor[patient.status] + '1a', color: statusColor[patient.status], borderColor: statusColor[patient.status] + '55' }">
                  <i class="pd__status-dot" :style="{ background: statusColor[patient.status] }"></i>{{ patient.status }}
                </span>
              </dd>
              <dt>治疗阶段</dt><dd>{{ treatmentPhaseText }}</dd>
              <dt>最近病情结论</dt><dd>{{ recentConclusion }}</dd>
            </dl>
          </div>
        </div>

        <!-- ② 患者旅程（核心时间轴） -->
        <div v-else-if="activeNav === 'journey'" class="pd__journey-wrap">
          <div class="pd__group pd__group--flush">
            <div class="pd__sec-head">
              <h3 class="pd__h pd__h--inline">诊疗时间轴 · 患者旅程</h3>
              <el-radio-group v-model="timelineFilter" size="small">
                <el-radio-button v-for="k in timelineKinds" :key="k" :value="k">{{ k }}</el-radio-button>
              </el-radio-group>
            </div>
            <el-timeline class="pd__tl">
              <el-timeline-item
                ...
                :hollow="node.kind === '检查'"
              >
                <div class="pd__tl-card" :style="{ borderLeftColor: kindColor[node.kind] }">
          ...
          <div class="pd__exams-bottom">
            ...
            <div class="pd__group">
              ...
              <div v-if="recentExams.length" class="pd__recent">
                <div v-for="e in recentExams" :key="e.id" class="pd__recent-row">
        ...
        <!-- ④ 病理与分子：诊断 → 分期 → 分子 → 报告 逻辑链 -->
        <div v-else-if="activeNav === 'pathology'">
          <div class="pd__chain">
            <div class="pd__chain-step">
              <div class="pd__chain-idx">1</div>
              <h4 class="pd__chain-t">病理诊断</h4>
              <div v-if="keyDiagnosis" class="pd__chain-body">
                <div class="pd__chain-main">{{ keyDiagnosis.histology }}</div>
                <div class="pd__chain-sub">{{ keyDiagnosis.differentiation }} · {{ keyDiagnosis.site }}</div>
                <div class="pd__chain-tag">来源：{{ keyDiagnosis.org }}</div>
              </div>
              <div v-else class="pd__chain-empty">待补全</div>
            </div>
            <div class="pd__chain-arrow">→</div>
            <div class="pd__chain-step">
              <div class="pd__chain-idx">2</div>
              <h4 class="pd__chain-t">分期依据</h4>
              <div v-if="keyDiagnosis" class="pd__chain-body">
                <div class="pd__chain-tnm">{{ keyDiagnosis.t }} {{ keyDiagnosis.n }} {{ keyDiagnosis.m }}</div>
                <div class="pd__chain-sub">分期 {{ keyDiagnosis.stage }}</div>
                <div class="pd__chain-tag">依据：{{ keyDiagnosis.basis }}</div>
              </div>
              <div v-else class="pd__chain-empty">待补全</div>
            </div>
            <div class="pd__chain-arrow">→</div>
            <div class="pd__chain-step">
              <div class="pd__chain-idx">3</div>
              <h4 class="pd__chain-t">分子标志物</h4>
              <div v-if="molecularCount" class="pd__chain-body">
            ...
            <div class="pd__chain-arrow">→</div>
            <div class="pd__chain-step">
              <div class="pd__chain-idx">4</div>
              <h4 class="pd__chain-t">检测报告</h4>
              <div v-if="patient.resources.filter((x) => x.type === '基因检测').length" class="pd__chain-body">
        ...
        <div v-else-if="activeNav === 'treatment'">
          <div v-if="patient.treatments.length" class="pd__txflow">
            <div v-for="(t, i) in patient.treatments" :key="t.id" class="pd__txnode" :class="{ 'is-doing': t.status === '进行中' }">
        ...
        <div v-else-if="activeNav === 'followup'">
          ...
          <div class="pd__group">
            ...
            <div v-if="patient.followUps.length" class="pd__fu-list">
              <div v-for="f in patient.followUps" :key="f.id" class="pd__fu-row">
      ...
      <!-- 右侧：仅患者旅程保留的当前阶段上下文 -->
      <aside v-if="activeNav === 'journey'" class="pd__ctx">
        <div class="pd__ctx-card">
          <div class="pd__ctx-head"><span class="pd__ctx-dot" style="background:#06b6d4"></span>当前阶段</div>
          <div v-if="currentTreatment" class="pd__ctx-body">
            <div class="pd__ctx-title">{{ currentTreatment.name }}</div>
            <div class="pd__ctx-sub">{{ currentTreatment.line }} · {{ currentTreatment.modality }} · {{ currentTreatment.scheme }}</div>
            <div class="pd__ctx-tags">
              <el-tag size="small" effect="light" :type="currentTreatment.status === '进行中' ? 'warning' : 'info'">{{ currentTreatment.status }}</el-tag>
              <span class="pd__ctx-meta">{{ currentTreatment.startDate }} ~ {{ currentTreatment.endDate === '—' ? '至今' : currentTreatment.endDate }}</span>
            </div>
          </div>
          <div v-else class="pd__ctx-empty">暂无进行中治疗</div>
        </div>
        <div class="pd__ctx-card">
          <div class="pd__ctx-head"><span class="pd__ctx-dot" style="background:#f59e0b"></span>最近事件</div>
          <div v-if="patient.timeline[0]" class="pd__ctx-body">
            <div class="pd__ctx-title">{{ patient.timeline[0].title }}</div>
            <div class="pd__ctx-sub">{{ patient.timeline[0].date || '待补全' }} · {{ patient.timeline[0].org }}</div>
            <div class="pd__ctx-desc">{{ patient.timeline[0].desc }}</div>
          </div>
          <div v-else class="pd__ctx-empty">暂无事件</div>
        </div>
      </aside>
    </div>
  </div>
